...
### 1. 外部 API 对接前必须分析文档，确认后再写代码

**背景**：曾将 Tushare weekly 接口的输入参数 `end_date` 误当作输出字段写入 `_FIELDS`，导致接口将该输入参数值回填到所有数据行，产生大量脏数据存入数据库。

**规则**：
- 对接任何外部 API（尤其是 Tushare）前，必须仔细区分**输入参数**和**输出字段**，两者不可混淆
- 接口有行数限制时（如 Tushare 单次最多 6000 行），必须提前思考最优的拉取策略：
  - 按日期区间分段（全市场数据）优于按实体（股票代码）逐个循环
  - 周线按7天/段、月线按31天/段，可确保全市场数据不超限
- 如有不确定的字段含义、分页策略或限制条件，**先向用户确认，再写代码**

### 2. 数据表结构变更前必须让用户确认

**背景**：曾多次在未与用户确认表结构的情况下直接写代码，导致字段设计反复变更。
